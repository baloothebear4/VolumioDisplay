...
# To add more screens, change this class, add the screen class
	...
	def tracksscreen( self ):
		...
		#print self.metadata
		with canvas( self.device ) as draw:
			draw.text((0,0),    self.metadata.artist,  font=self.font, fill=255)
			draw.text((0,15),    self.metadata.song,  font=self.font, fill=255)
	...
	def volumioscreen( self ):
		...
		with canvas( self.device ) as draw:
			...
			draw.pieslice( (x, y, x+CONTROLS_WIDTH, y+CONTROLS_WIDTH), 135, int(135+(270*vol/100)), outline=255, fill=255)
			i = 7
			draw.ellipse( (x+i, y+i, x+CONTROLS_WIDTH-i, y+CONTROLS_WIDTH-i), outline=255, fill=0)
			t = 6
	...
	def systemscreen( self ):
		...
		with canvas( self.device ) as draw:
			...
			self.bar_meter( draw, 0, 0, int(self.sys.status['cpu_load']), "CPU", sfont)
			self.bar_meter( draw, 0, 15, int(self.sys.status['mpd_load']), "MPD", sfont)
			self.bar_meter( draw, 0, 30, int(self.sys.status['shairport_load']), "Shair", sfont)
			self.bar_meter( draw, 0, 45, int(self.sys.status['disk_used']), "Disk", sfont)
