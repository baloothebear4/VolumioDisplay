...
// Baloothebear 4 - add VolumioDisplay configfile access functions
...
function safefilerewrite($fileName, $dataToSave) {
	if ($fp = fopen($fileName, 'w')) {
		...
		//file was locked so now we can store information
			if ($canWrite) {
				$status = fwrite($fp, $dataToSave);
				if( $status == FALSE) {
					$_SESSION['debugVD'] .= "VolumioDisplay config file write failed ";
				}
				flock($fp, LOCK_UN);
			}
		fclose($fp);
	} else {
		$_SESSION['debugVD'] .= "write open failure";
	}
}
...
function read_VolumioDisplay_config( $file ) {
  //$_SESSION['debugVD'] .= "::Reading VD config -->".$file. ". ";
	try {
	    $config = parse_ini_file($file, true);
	} catch (Exception $e) {
			$_SESSION['debugVD'] .=" ::VolumioDisplay config read exception >>".$e->getMessage(). "\n";
	}

	if ($config != FALSE) {
		$res = array();
		foreach($config['screens_available'] as $key => $val) {
			if(is_array($val)) {
				$res[] = "[$key]";
				foreach($val as $skey => $sval)
					$res[] = "$skey = ".(is_numeric($sval) ? $sval : '"'.$sval.'"');
		...
		//$_SESSION['debugVD'] .= " ::Screens_available=>>".implode( $res );
		//$_SESSION['debugVD'] .= " ::Duration>>".$config['settings']['screen_duration'];
		//$_SESSION['debugVD'] .=" ::Intro>>".$config['intro'];

	}
	else {
		$_SESSION['debugVD'] .="VolumioDisplay config file parse failed ";
	}
